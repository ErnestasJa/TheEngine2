...
inline void SetUniformMat4(const gpu_shader_uniform_handle& handle, const float* value, int count,
                           bool transpose = false)
{
                               ...
                               void* data)
{
  glBufferData(handle.buffer_type, buffer_size * handle.component_count * handle.component_size,
               data, handle.usage_hint);
}

inline void UpdateBufferObjectSubData(const gpu_buffer_object_handle& handle, uint32_t offset,
                                      uint32_t size, void* data)
{
  glBufferSubData(handle.buffer_type, offset * handle.component_count * handle.component_size,
                  size * handle.component_count * handle.component_size, data);
}
...
inline void ProcessHandle(const BufferDescriptor& desc, gpu_buffer_object_handle& handle)
{
  ...
  handle.usage_hint      = GetUsageHint();
}
} // namespace gl
} // namespace render
