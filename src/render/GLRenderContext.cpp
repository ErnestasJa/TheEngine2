...
std::shared_ptr<IRenderContext> CreateContext(const SWindowDefinition& def)
{
    ...
    return std::make_shared<GLRenderContext>(std::move(windowModule), std::move(window),
                                             std::move(renderer));
}

GLRenderContext::GLRenderContext(std::unique_ptr<DefaultWindowModule>&& windowModule,
