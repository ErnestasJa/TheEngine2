...
void DebugLineMesh::AddGrid(int count, int spacing, glm::tvec3<uint8_t> color, glm::vec3 offset)
{
  count = count + (count % 2);
  ...
  for (int i = 0; i < count; i++) {
    float lineOffset = start + (i * spacing);
    AddLine(glm::vec3(start, 0, lineOffset) + offset, glm::vec3(end, 0, lineOffset) + offset, color);
    AddLine(glm::vec3(lineOffset, 0, start) + offset, glm::vec3(lineOffset, 0, end) + offset, color);
  }
}
